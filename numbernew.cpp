...
int countaccept=0;  //count accept of ion moving
int countnc=0;  //count accept of crowder moving
...
int move(int i,double *x1,double *y1,double *z1,double *q1,double *x2,double *y2,double *z2,double *q2)  //function definition
{
	...
	for (k=0;k<nall;k++){
		...
		else if(k>=(n1+n2+n3+n4)&&i>=(n1+n2+n3+n4)){if(d_ii<(rc+rc)) return 22;}			else {if(d_ii<(ri+rc)) return 23;}
		}
	}
	////metroplis
